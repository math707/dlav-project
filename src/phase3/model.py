"""Phase 3 models for sim-to-real trajectory planning."""
...
def _build_resnet18_spatial_backbone(pretrained: bool, strict_pretrained: bool = False):
    try:
        ...
        raise ImportError(
            "Phase 3 ResNet18 planner requires torchvision. Install torchvision to use MODEL_NAME='phase3_resnet18'."
        ) from exc
...
class CameraTensorPreprocessor(nn.Module):
    def __init__(self, normalize_to_unit_scale: bool = True, imagenet_normalize: bool = True):
        super().__init__()
...
class Phase3Planner(nn.Module):
    """ResNet18-based Phase 3 planner using camera and motion history only."""
    ...
    def __init__(
        ...
        *,
        ...
        strict_pretrained_backbone: bool = False,
        ...
        fused_dim = visual_embedding_dim + history_embedding_dim
    ...
    def forward(self, camera, history):
        visual_embedding = self._encode_visual(camera)
        ...
        return trajectory.reshape(-1, self.future_steps, self.trajectory_features)


MODEL_REGISTRY = {
    'phase3_resnet18': Phase3Planner,
}


def build_model(name: str = 'phase3_resnet18', **kwargs):
    try:
