...
def normalize_phase_name(phase_name: str | None, default: str = 'phase1') -> str:
    return sanitize_run_name(phase_name, default_name=default)
...
def create_run_context(
    ...
    runs_root = outputs_dir / 'runs' / resolved_phase_name
    ...
    if environment == 'colab' and is_google_drive_mounted():
        resolved_drive_root = Path(drive_root or DEFAULT_COLAB_DRIVE_ROOT) / resolved_phase_name
    ...
    return RunContext(
        ...
        submission_path=run_dir / f'submission_{resolved_phase_name}.csv',
...
def sync_run_to_drive(run_context: RunContext) -> Path | None:
    ...
    drive_root = run_context.drive_root or (DEFAULT_COLAB_DRIVE_ROOT / run_context.phase_name)
